...
- Markdown はブロック要素（段落・見出し・リスト等）単位でダブルクリックしてセット
- テキスト/コードは行番号ガターまたは本文行のいずれかをダブルクリックして行単位でセット（シングルクリックはテキスト選択やリンク遷移のため無反応）
- マーカーは背景色で表示（Markdownは左黄帯＋薄い背景、コード/テキストはガターの行番号セルを黄色反転＋本文行は薄い背景）
- 再オープン時は自動スクロールしない。ツールバーに「しおりへ」ボタンを設置し、クリックでジャンプ
- 1ファイルにつきしおりは1つだけ（しおり方式）。同位置の再ダブルクリックで解除、別位置ダブルクリックで移動
...
- `marked.parse()` 後、`#md-body` の直下の子要素に順番に `data-bm-idx="0..N"` を付与し、全ブロックに `.bm-target` クラス（`cursor: pointer; touch-action: manipulation`）を付与
- ダブルクリックでサーバーに PUT / 同じ所なら DELETE（ブロック内のリンク・ボタン・画像等の上ではイベントを無視）
- しおり済みブロックには `.bookmarked` クラスを付与し、左4px帯＋薄い黄色背景で表示（アイコンなし）
...
- ガター列または本文行のいずれかをダブルクリックでセット/解除。`touch-action: manipulation` でモバイルのダブルタップズームを抑止。シングルクリック時はテキスト選択・リンク操作を阻害しない
- CDN の `pre code.hljs { padding: 1em }` が本文をガター基準線から押し下げるため、`.code-body code.hljs { padding: 0 !important; }` で打ち消す
- しおり行は `.bookmarked` クラスでガター側の行番号セルを黄色反転＋本文側は薄い黄色背景（本文側の左帯は入れない。テキストと重なり読みにくいため）

### ツールバー

セクションヘッダー（ファイル名を表示している部分、既存 `btn-edit` と同じ場所）の右側に「しおりへ」ボタン（テキストラベルのみ）を配置する。

- しおりあり かつ 該当要素がDOM上に存在する場合: ボタン表示、クリック（シングルクリック）で該当要素に `scrollIntoView({block: 'center'})`
- しおりなし、または しおり index が要素数を超える（ファイル編集でずれた）場合: ボタン非表示
...
showBlob(path)
  ...
  ├─ レンダリング (md or code)
  ├─ bookmark あり かつ 対象要素あり → 該当要素に .bookmarked、ジャンプボタン表示
  │  bookmark あり かつ 対象要素なし → マーカー・ボタンとも非表示（データは温存）
  └─ ダブルクリックハンドラ登録
...
- しおり取得失敗: silent（`try/catch` で空オブジェクトにフォールバック）。プレビュー自体は継続
- しおり保存/削除失敗: `alert()` で1回通知（プロジェクトの既存エラーUX `saveBlob` に合わせる）
- `bookmarks.json` 破損: サーバーは空辞書で起動
- ファイル編集でインデックスが要素数を超えた場合: マーカー描画と「しおりへ」ボタン表示を共にスキップ。しおりデータ自体は残し、ユーザーが新しい位置をセットし直すまで保持
...
- `static/style.css`: `.bm-target`、`.bookmarked`、`.code-wrapper`、`.code-gutter`、`.code-line-num`、`.code-body`、`.code-line`、`.btn-jump`、および `.code-body code.hljs { padding: 0 !important }` 打ち消し
